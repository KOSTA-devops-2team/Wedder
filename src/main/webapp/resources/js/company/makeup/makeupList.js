$(document).ready(function () {
    ...
    function performSearch() {
        ...
        const url = keyword ? '/makeup/search' : '/makeup/page'; // 검색어가 있으면 POST, 없으면 GET
        ...
        $.ajax({
            ...
            success: function (data, status) {
                if (data.makeupList.length > 0) {
                    ...
                    searchCompanyList(data.makeupList);
    ...
    function goToPage(page) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                searchCompanyList(response.makeupList);
    ...
    // 검색 결과를 처리하는 함수
    function searchCompanyList(makeupList) {
        // makeupList가 null이나 undefined일 경우 빈 배열로 설정
        makeupList = makeupList || [];
        // $('.list-all').empty();
        let companyListHtml = '';
        makeupList.forEach(function (company) {
            companyListHtml += `
                        <div class="card">
                        <a href="/makeup/detail/${company.companyId}">
                        ...
                        <div class="card-info">
                            ...
                            <div class="makeup-type">상품 유형 : ${company.makeupDto.makeupType}</div>
                            <div class="position">시술자 직급 : ${company.makeupDto.position}</div>
                        </div>
                        </div>`;
        })
